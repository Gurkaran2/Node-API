...
--
-- Dumping routines for database 'dbs'
--
/*!50003 DROP PROCEDURE IF EXISTS `SumPowerSP1` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SumPowerSP1`()
BEGIN
...
select sum(power) from site_power group by dsu_id;
 ...
 UPDATE dsu_power  
  SET total_power = 
    CASE WHEN  dsu_id = v_id THEN v_power ELSE 0 END,
 time_aggregated = NOW() where  dsu_id = v_id ;
...
-- Dump completed on 2019-07-05  1:47:43
